    ...
    try {
        ...
        const items = invites.map(m => {
            ...
            return {
                ...
                senderRole: m.sender?.role === 'hod' ? 'HOD' : m.sender?.role === 'managing_authority' ? 'Principal' : '',
        ...
        // Check each meeting's live status. When the HOD/Principal taps End, the
            ...
            return `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:1rem; border:1px solid #e2e8f0; border-radius:10px; margin-bottom:0.75rem;">
                    <div>
                        ...
                        <div style="color:#94a3b8; font-size:0.82rem;"><i class="fas fa-user"></i> from ${it.from}${it.senderRole ? ` (${it.senderRole})` : ''}  ${it.time}</div>
                    </div>
                    ${actionHtml}
                </div>
            `;
...
// Auto-refresh teacher meeting invitations every 8s so the Join button flips to
// "Completed" shortly after the HOD/Principal ends the meeting (no manual refresh).
let _teacherInvitesPollTimer = null;
function startTeacherInvitesPoll() {
    if (_teacherInvitesPollTimer) return;
    _teacherInvitesPollTimer = setInterval(() => {
        const container = document.getElementById('teacherMeetingInvites');
        if (container && container.offsetParent !== null) {
            loadTeacherMeetingInvites();
        }
    }, 8000);
}
...
async function startClassVirtual() {
    ...
    try {
        ...
        // Note: 15-minute restriction is enforced by the backend.
        // The frontend no longer blocks early - if backend allows it, we proceed.
